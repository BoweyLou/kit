...
For manual startup in this source repo, use:

```text
Read AGENTS.md, REVIEW.md, and .agent-workflows/README.md.
Then follow .agent-workflows/repo-review.md in bootstrap mode.
Use the companion repo-contract-kit repo's workflows directory for new workflow-source edits.
Treat workflows/prompts/ as legacy/archive material in this checkout and .codex/prompts/ as its generated Codex mirror.
...
This checkout is legacy `agent-workflow-kit` source history. New prompt,
persona, schema, task-packet, and research workflow source edits belong under
the companion `repo-contract-kit` repo's `workflows` directory.

The local `workflows/prompts/` tree remains archive and migration evidence, and
`.codex/prompts/` remains its generated Codex mirror for legacy validation. Edit
that legacy tree only when a task explicitly calls for archive maintenance or
migration evidence, then run `make prompt-adapters-export` and `make validate`.
`repo-contract-kit` is installed here to dogfood the target-repo guardrails and
command surface, not to make this checkout the live workflow source again.
