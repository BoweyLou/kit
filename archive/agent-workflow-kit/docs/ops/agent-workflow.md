...
goal-check summary, and a receipt template. Discovery check failures are
recorded as warnings so inherited or messy repos can still be reviewed.
...
KIT=/path/to/repo-contract-kit` compares against a local checkout and
...
`make kit-migrate-config KIT=/path/to/repo-contract-kit` to apply only the
...
`make kit-refresh KIT=/path/to/repo-contract-kit` is the legacy local-checkout
...
explicit unknowns when no area contract matches the proposed scope.
...
into a machine-readable task packet scaffold.
...
- `.doc-contract-kit/make/repo-contract.mk` is the managed make target fragment
  installed in ordinary target repos. This legacy dogfood checkout keeps an
  explicit target-owned root `Makefile`; documented local commands must have
  wrappers there, and `tests/test_documented_make_targets.py` guards that list.
