...
# Script flow:
# 1. Resolve the current target repo, repo-contract-kit checkout, and workflow source repo.
# 2. Optionally fast-forward the repo-contract-kit checkout.
...
def discover_kit(target: Path, explicit: str):
    ...
    raise SystemExit("Set KIT=/path/to/repo-contract-kit or REPO_CONTRACT_KIT; no local kit checkout was discovered.")
...
def parse_args():
    ...
    parser.add_argument("--kit", default="", help="repo-contract-kit checkout. Defaults to KIT/REPO_CONTRACT_KIT or local discovery.")
