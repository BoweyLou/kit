...
EFFORT_DURATION_MS_FIELDS = (
    "duration_ms",
    "first_finding_latency_ms",
    "time_to_green_ms",
)
EFFORT_COST_FIELDS = ("estimated_cost_usd",)
EFFORT_NUMBER_FIELDS = ("human_review_minutes",)
...
def validate_non_negative_integer(value: Any, path: str, errors: list[str]) -> None:
    require(is_non_negative_integer(value), f"{path} must be a non-negative integer", errors)


def validate_rate(value: Any, path: str, errors: list[str]) -> None:
    require(is_finite_number(value) and 0 <= value <= 1, f"{path} must be a number from 0 to 1", errors)


def validate_non_negative_number(value: Any, path: str, errors: list[str]) -> None:
    require(is_non_negative_number(value), f"{path} must be a non-negative number", errors)


def validate_harness_metrics(harness_metrics: Any, errors: list[str]) -> None:
    ...
    for field in ("context_file_count", "commands_run_count", "changed_file_count"):
        if field in harness_metrics:
            validate_non_negative_integer(harness_metrics[field], f"harness_metrics.{field}", errors)

    review_outcome = harness_metrics.get("review_outcome")
    if review_outcome is not None:
        require(isinstance(review_outcome, dict), "harness_metrics.review_outcome must be an object when present", errors)
        if isinstance(review_outcome, dict):
            for field in REVIEW_OUTCOME_COUNT_FIELDS:
                if field in review_outcome:
                    validate_non_negative_integer(
                        review_outcome[field],
                        f"harness_metrics.review_outcome.{field}",
                        errors,
                    )
            for field in REVIEW_OUTCOME_RATE_FIELDS:
                if field in review_outcome:
                    validate_rate(review_outcome[field], f"harness_metrics.review_outcome.{field}", errors)

    effort = harness_metrics.get("effort")
    if effort is not None:
        require(isinstance(effort, dict), "harness_metrics.effort must be an object when present", errors)
        if isinstance(effort, dict):
            for field in EFFORT_COUNT_FIELDS:
                if field in effort:
                    validate_non_negative_integer(effort[field], f"harness_metrics.effort.{field}", errors)
            for field in EFFORT_DURATION_MS_FIELDS:
                if field in effort:
                    validate_non_negative_integer(effort[field], f"harness_metrics.effort.{field}", errors)
            for field in EFFORT_COST_FIELDS:
                if field in effort:
                    validate_non_negative_number(effort[field], f"harness_metrics.effort.{field}", errors)
            for field in EFFORT_NUMBER_FIELDS:
                if field in effort:
                    validate_non_negative_number(effort[field], f"harness_metrics.effort.{field}", errors)
...
def validate_receipt(receipt: Any, strict: bool) -> list[str]:
    ...
    validate_harness_metrics(receipt.get("harness_metrics"), errors)
