...
- `templates/review-map.md`: Organizes large changesets into changed-file
  ...
  `make context-packet` or installed context bundles for navigation; it does
...
- `schemas/review-map.schema.json`: Machine-readable review-map artifact
  schema mirroring `templates/review-map.md` without requiring private
  transcript data, hosted-service fields, or review-runner mutation.
...
`agent-start` latest-ADR evidence, context-packet or context-bundle ADR items,
...
reviewers. Start from `make context-packet` or an installed context bundle,
