...
boundaries, or closeout evidence.
...
- For large reviews or implementations that still belong to one approved task,
  generate bounded phase files instead of stuffing every step into one session.
  Use phase files only to preserve fresh session continuity; they do not replace
  the task packet, widen scope, or authorize work outside the packet.
- Include `phase_files` when needed:
  `needed`, `reason`, optional `base_path`, and a short ordered `phases` list.
  Each phase should have `id`, `title`, `objective`, `allowed_scope`,
  `completion_evidence`, and optional `handoff_notes`.
- Keep each phase independently resumable: name source inputs, allowed files,
  validation evidence, stop conditions, and what must be true before the next
  phase starts.
- Do not create phase files for small work where the task packet itself is
  enough context.
