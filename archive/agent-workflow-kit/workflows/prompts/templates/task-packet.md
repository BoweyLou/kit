...
- phase_files:
- needed: `yes | no`
- reason: `<why this task needs or does not need phase files>`
- base_path: `<directory or sidecar path for phase files, or not-applicable>`
- phases:
  - id: `<phase-id>`
    title: `<short phase title>`
    objective: `<bounded objective for a fresh agent session>`
    allowed_scope:
      - `<files or directories this phase may touch>`
    completion_evidence:
      - `<command output, diff, receipt, generated artifact, or review evidence required before next phase>`
    handoff_notes: `<what the next fresh session needs to know, including stop conditions>`

Use phase files only when the task is too large for one fresh session but still
belongs to one approved task packet. They preserve continuity; they do not
replace the task packet, widen scope, or approve unrelated work.
