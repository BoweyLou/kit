...
This is the install-layer view. `repo-contract-kit` owns the public installer,
global CLI, and installed target-repo surfaces that make the harness executable.
Vendored prompts, schemas, and policies reach target repos through explicit
`repo-contract-kit` updates; workflow-source details are maintainer-only while
the split remains.
...
| Task status | `make agent-task-status` | active local tasks, worktrees, dirty or stale metadata, unknown scopes, overlaps, owner/session/thread/automation attribution, heartbeat, lease expiry, linked receipts, and `parallel_context.can_start_write_task` | status text or JSON/strict failure |
...
| Task cleanup | `make agent-task-cleanup`, `make agent-task-closeout` | existing flat/nested task-worktree layout, stale task reconciliation, and finished sibling-worktree retention | dry-run inventory, explicit nested move action, guarded closeout removal, task dispositions such as `clean-to-close`, `dirty-needs-review`, `missing-worktree`, and `blocked-by-receipt` |
| Task worktree | `make agent-task-prepare TASK=<id> SCOPE=<paths>` | isolated branch and sibling worktree for write-capable agents, with optional `DIRTY_PRIMARY_BASELINE=1` baseline capture for pre-existing primary dirt and balanced parallel-coordination blocking for same-scope hazards | task packet, receipt template, in-flight metadata, and `parallel_context` |
...
| Supervised learning records | `kit learn status`, event/proposal/decision/context routes, `kit learn thread-summary import/list`, `kit learn upstream export/list/reconcile`, `kit learn evaluate`, `.agent-workflows/learning-policy.json`, `schemas/learning-*.schema.json` | target-owned supervised policy gates bounded local evidence, one explicit redacted aggregate import, and portable source-review candidates | import never scans history or raw interaction content and writes one approved sidecar event only; upstream export needs current approved lineage, redaction confirmation, and public-ok/internal privacy; lists/evaluate/reconcile are read-only, revalidate lineage, and never propagate changes |
...
4. Does the change belong in `repo-contract-kit` or the target repo?
...
- target-repo scripts
- managed templates
- workflow prompt or schema snapshots generated from `workflows/`
...
Start in the target repo when changing:
...
- keep improving context-bundle consumers so workers use compact deterministic
  reports before broad repo rereads
- expand docs-as-tests only through explicit local config and additional
  high-confidence local assertion kinds
- tune token-budget and context-economy metrics once more target repos have
  warning data
