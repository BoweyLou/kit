...
| Supervised learning records | `kit learn status`, `kit learn event record/list`, `kit learn proposal create/list`, `kit learn decision record/list`, `kit learn context build/list`, `.agent-workflows/learning-policy.json`, `schemas/learning-*.schema.json` | target-owned policy gates bounded sidecar learning artifacts | context build requires a valid approved decision and linked approved proposal; list/bundle revalidate lineage, cap safe guidance, and never expose raw event/evidence/feedback/conversation content or execute a recommendation |
